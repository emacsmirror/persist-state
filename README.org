...
- [[github.com/meedstrom/eager-state][eager-state]] :: Most similar to persist-state itself, but the implementation differs.  It has no default set of persistence functions, aiming instead to make it easy to add them.
