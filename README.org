...
: git clone https://codeberg.org/meedstrom/emacs-persist-state.git
...
- [[https://github.com/meedstrom/eager-state][eager-state]] :: Most similar to persist-state itself, but the implementation differs.  It has no default set of persistence functions, aiming instead to make it easy to add them.
...
- Fix =persist-state--maybe-save-eshell= (it did not work)
...
- Issue reports: [[https://codeberg.org/meedstrom/emacs-persist-state/issues][Codeberg]]
- Mastodon: [[https://hachyderm.io/@meedstrom][@meedstrom@hachyderm.io]]
- Mail: see =git log=.
